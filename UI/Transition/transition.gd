...
func exit_screen():
	...
	start.tween_property($Screen, "position", Vector2(-2000, 0), 1.75).set_ease(Tween.EASE_IN)
...
func enter_screen():
	...
	end.tween_property($Screen, "position", Vector2(0, 0), 1.75).set_ease(Tween.EASE_OUT)
	await end.finished
